 ...
 * 
 */
public class TableScanerRegistrar implements ImportBeanDefinitionRegistrar {

	private static Logger log = LoggerFactory.getLogger(TableScanerRegistrar.class.getName());

	@Override
	public void registerBeanDefinitions(AnnotationMetadata importingClassMetadata, BeanDefinitionRegistry registry) {
		AnnotationAttributes mapperScanAttrs = AnnotationAttributes
				.fromMap(importingClassMetadata.getAnnotationAttributes(TableEntityScan.class.getName()));
		...
		if (basePackages.size() > 0) {
			...
			log.info("扫描TableName数据库实体注册数量" + count + ":" + basePackagesLog);
		}
	}
