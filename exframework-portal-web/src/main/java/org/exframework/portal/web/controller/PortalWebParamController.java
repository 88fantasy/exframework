...
public class PortalWebParamController {
	...
	public ApiResponseData<String> getParamValue(
		...
		if (!StringUtils.hasText(key) && !StringUtils.hasText(account)) {
			return new ApiResponseData<String>(portalCoreSystemParameterService.getAccoutParameter(key, account));
		} else {
	...
	public ApiResponseData<Boolean> post(
		...
		return new ApiResponseData<Boolean>(portalCoreSystemParameterService.putAccountKey(dto.getAccount(), dto.getCode(), dto.getName(), dto.getValue(), dto.getDescription()));
	}
}
