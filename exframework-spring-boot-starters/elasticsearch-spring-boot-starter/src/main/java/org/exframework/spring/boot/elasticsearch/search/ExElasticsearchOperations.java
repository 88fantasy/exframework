package org.exframework.spring.boot.elasticsearch.search;
...
//						wrapper.apply(value);
	...
	default <E,T> List<E> list(Collection<FilterCondition> params, Class<T> documentClass, Class<E> viewClass) {
		...
		return searchForStream(query, documentClass).stream().map(row -> BeanUtils.copyTo(row.getContent(), viewClass)).collect(Collectors.toList());
	}
}
